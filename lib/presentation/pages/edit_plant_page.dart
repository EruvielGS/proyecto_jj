...
class _EditPlantPageState extends State<EditPlantPage> {
  ...
  Future<void> _initData() async {
    if (!_isInitialized) {
      ...
      try {
        ...
        typeController.text = plant.type;
        selectedDeviceId = plant.deviceId;
  ...
  Future<void> _updatePlant() async {
    ...
    try {
      ...
      final plant = await plantProvider.updatePlant(
      ...
      if (plant != null) {
        if (mounted) {
          AlertHelper.showSuccessAlert(
              context, 'Planta actualizada correctamente');
      ...
      if (mounted) {
        AlertHelper.showErrorAlert(
            context, 'Error al actualizar planta: ${e.toString()}');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    if (isLoading || plant == null) {
      return Scaffold(
        appBar: AppBar(title: Text('Editar Planta')),
